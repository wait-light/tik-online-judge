 ...
 */
@RestController
@RequestMapping("/social/task-item")
public class TaskItemController {
    ...
    public CommonResult list(){
        PageUtils.makePage();
        List<TaskItem> list = taskItemService.list();
        return CommonResult.success().listData(list);
    }

    @PostMapping("")
    public CommonResult save(@RequestBody TaskItem entity) {
        return  taskItemService.save(entity) ?
    ...
    public CommonResult deleteBatch(@RequestBody Long[] ids){
        return taskItemService.removeByIds(Arrays.asList(ids)) ?
    ...
    public CommonResult delete(@PathVariable("id") Long id){
        return taskItemService.removeById(id) ?
            CommonResult.success().setMsg("删除成功") :
            CommonResult.error().setMsg("删除失败");
    }

    @PutMapping("/{id}")
    public CommonResult update(@RequestBody TaskItem entity){
        return taskItemService.updateById(entity) ?
    ...
    public CommonResult info(@PathVariable("id") Long id){
        TaskItem entity = taskItemService.getById(id);
        return entity != null ?
